...
import { SECOND_IN_MS } from '@const/index';
...
export class AdminController {
  ...
  @Get('/:adminId')
  @HttpCode(HttpStatus.OK)
  @UseGuards(RoleGuard(E_Role.SuperAdmin, E_Role.Seller))
  ...
  async login(@Body() body: LoginDto) {
    ...
    const foundAdmin =
      await this.adminService.getAdminByLoginOrEmail(loginOrEmail);
  ...
  async registerIndividual(@Body() body: RegisterDto) {
    ...
    const adminWithLogin = await this.adminService.getAdminByLoginOrEmail(
      body.login,
    );
    const adminWithEmail = await this.adminService.getAdminByLoginOrEmail(
      body.email,
    );
